{% docs dg_self_service_overview %}

## Self-service — Digital Gaming
...
- **Nommage métier** en `snake_case` minuscule (ex: `game_sales_analysis`), sans préfixe `FTC_`/
  `DIM_` — signale clairement "vue de consommation" vs "table du star schema".
...
- **Schéma dédié** (`DB_WH_DIGITAL_GAMING_SELF_SERVICE`) pour pouvoir donner un accès en lecture
  aux outils BI sans exposer le schéma interne `DB_WH_DIGITAL_GAMING`.

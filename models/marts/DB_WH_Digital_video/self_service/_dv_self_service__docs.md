{% docs dv_self_service_overview %}

## Self-service — Digital Video
...
- **Nommage métier** en `snake_case` minuscule (ex: `movie_ratings_analysis`), sans préfixe
...
- **Schéma dédié** (`DB_WH_DIGITAL_VIDEO_SELF_SERVICE`) pour pouvoir donner un accès en lecture
  aux outils BI sans exposer le schéma interne `DB_WH_DIGITAL_VIDEO`.
